...
<?php
  ...
  if( isset($_GET['did']))
  {
    ...
    $del="DELETE FROM add_whyiip where id= '".$_GET['did']."'";
  ...
  for ($i=0; $i<$totdel ; $i++)
   { 
       $del="DELETE FROM add_whyiip where id= '".$_POST['c1'][$i]."'";
       $exe=mysqli_query($conn,$del); 
       echo "hlo";
   }
     ...
     <div class="body">
        ...
        <form method="post">
            <table border="2" style="border-collapse: collapse;width: 100%;">
                 ...
                 <?php 
                        $sel="SELECT*from add_whyiip $where";
                        $exe=mysqli_query($conn,$sel);
                        while($fetch=mysqli_fetch_array($exe))
                         {
                  ?>        <tr>
                                <td>
                                    <input type="checkbox" name="c1[]" value=" <?php echo $fetch['id'] ?>">
                                </td>
                                <td><?php echo $fetch['id']?></td>
                                <td><?php echo $fetch['title']?></td>
                                <td>
                                    <a href="viewwhyiip.php?did=<?php echo $fetch['id']?>"> Delete </a>
                                </td>
                                <td>
                                    <a href="addwhyiip.php?nid=<?php echo $fetch['id'] ?> "> Edit </a>
                                </td>
                            </tr>   
