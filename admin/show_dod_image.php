<?php
	...
	if(isset($_GET['del'])){
		$ok=$_GET['del'];
		echo "$ok";
		$delete="DELETE FROM design_of_day WHERE id='".$ok."'";
		$result=$conn->query($delete);
		if($result==true){
			$sdel="Row Deleted";
		}else{
			$ndel="Row Is Not Delete";
		}
	}
	//Multiple Delete Row
	if(isset($_POST['multiple_delete'])){	
		...
		for($i=0;$i<$total;$i++){
			$del="DELETE FROM design_of_day WHERE id='".$_POST['box'][$i]."'";
			$conn->query($del);
		}	
	}
	//search name
	if($_GET['search']==404){
		$name=$_POST['search'];
		echo $name;
		$selectt="SELECT * FROM design_of_day WHERE id='".$name."'";
			...
			<div class="container">
				...
				<form action="" method="post">
					<div class="container">
						...
						<button type="submit" class="btn btn-success" name="asc">Ascending</button>
						<button type="submit" class="btn btn-success" name="dsc">Descending</button>
						<button type="submit" class="btn btn-success" name="nam">Name</button>
						...
						<input type="text" name="search" placeholder="Search..."><a href="show_dod_image.php?search=404"><button type="submit" class="btn btn-light" name="Show">Search</button></a>
					</div>
					<table class="table table-striped">
						...
						<?php
							...
							$select="SELECT * FROM design_of_day WHERE status='".$where."' or status='".$one."' LIMIT $start,$limit";
							$result=$conn->query($select);
							if($result->num_rows >0){
								while($row=$result->fetch_assoc()){
									$path1="../dobfirst/".$row['first_image'];
									//echo $path1;
									$path2="../dobsecond/".$row['second_image'];
									//echo $path2;
									?>
										<tbody>
											<tr>
												<td>&ensp;<?php echo $row['id'] ?></td>
												<td><img src="<?php echo $path1; ?>" width='50px' height='50px' alt="no img"></td>
												<td><?php echo $row['price1'] ?></td>
												<td><?php echo $row['detail1'] ?></td>
												<td><img src="<?php echo $path2; ?>" width='50px' height='50px' alt="no img"></td>
												...
												<td><a href="show_dod_image.php?del=<?php echo $row['id']?>">Delete</a></td>
												<td><a href="add_dod_image.php?edit=<?php echo $row['id']?>">Edit</a></td>
											</tr>
										</tbody>
									<?php
								}
				...
				<table align="center" width="30%">
					<tr>
						<td><a href="show_dod_image.php?page=0">Frist</a></td>
						<td><a href="show_dod_image.php?page=<?php echo $pre; ?>">Pre</a></td>
						<td>
							<?php
								for($i=1;$i<=$last;$i++){
									if($i>3){
							?>
										<a href="show_dod_image.php?page=<?php echo $i-1; ?>">.</a>
							<?php
										
									}else{
							?>
										<a href="show_dod_image.php?page=<?php echo $i-1; ?>"><?php echo $i; ?></a>
							<?php			
									}
							?>	
							<?php
						...
						<td><a href="show_dod_image.php?page=<?php echo $next; ?>">Next</a></td>
						<td><a href="show_dod_image.php?page=<?php echo $last-1; ?>">Last</a></td>
					</tr>
				</table>
				</form>
			</div>
