<?php
	...
	if(isset($_GET['del'])){
		$ok=$_GET['del'];
		echo "$ok";
		$delete="DELETE FROM kids_shirt WHERE id='".$ok."'";
	...
	if(isset($_POST['multiple_delete'])){	
		...
		for($i=0;$i<$total;$i++){
			$del="DELETE FROM kids_shirt WHERE id='".$_POST['box'][$i]."'";
			$conn->query($del);
		}	
	}
	//search name
	if($_GET['search']==404){
		$name=$_POST['search'];
		echo $name;
		$selectt="SELECT * FROM kids_shirt WHERE id='".$name."'";
			...
			<div class="container">
				...
				<form action="" method="post">
					<div class="container">
						...
						<input type="text" name="search" placeholder="Search..."><a href="show_kids_shirt.php?search=404"><button type="button" class="btn btn-light" name="Show">Search</button></a>
					</div>
					...
					<table class="table table-striped">
						...
						<?php
							...
							$select="SELECT * FROM kids_shirt WHERE status='".$where."' or status='".$one."' LIMIT $start,$limit";
							$result=$conn->query($select);
							if($result->num_rows >0){
								while($row=$result->fetch_assoc()){
									$path="../products_images/kidshirt/".$row['main_image'];
									$path1="../products_images/kidshirt/".$row['first_sub_image'];
									$path2="../products_images/kidshirt/".$row['second_sub_image'];
									$path3="../products_images/kidshirt/".$row['third_sub_image'];
									$path4="../products_images/kidshirt/".$row['fourth_sub_image'];
									$path5="../products_images/kidshirt/".$row['fifth_sub_image'];
									?>
										<tbody>
											<tr>
												...
												<td><a href="show_kids_shirt.php?del=<?php echo $row['id']?>">Delete</a></td>
												<td><a href="add_kids_shirt.php?edit=<?php echo $row['id']?>">Edit</a></td>
											</tr>
										</tbody>
									<?php
								}
				...
				<table align="center" width="30%">
					<tr>
						<td><a href="show_kids_shirt.php?page=0">Frist</a></td>
						<td><a href="show_kids_shirt.php?page=<?php echo $pre; ?>">Pre</a></td>
						<td>
							<?php
								for($i=1;$i<=$last;$i++){
									if($i>3){
							?>
										<a href="show_kids_shirt.php?page=<?php echo $i-1; ?>">.</a>
							<?php
										
									}else{
							?>
										<a href="show_kids_shirt.php?page=<?php echo $i-1; ?>"><?php echo $i; ?></a>
							<?php			
									}
							?>	
							<?php
						...
						<td><a href="show_kids_shirt.php?page=<?php echo $next; ?>">Next</a></td>
						<td><a href="show_kids_shirt.php?page=<?php echo $last-1; ?>">Last</a></td>
					</tr>
				</table>
				</form>
			</div>
