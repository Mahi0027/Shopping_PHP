<?php
	...
	$path="../products_images/menshirt/".$name;
	...
	if($_GET['edit']==""){
		if(isset($_POST['save'])){
			if(($image_title!="") && ($set_prise!="")){
				$sql="INSERT INTO men_shirt(name,main_image,first_sub_image,second_sub_image,third_sub_image,fourth_sub_image,fifth_sub_image,price,detail,status) VALUES ('$image_title','$name','$name1','$name2','$name3','$name4','$name5','$set_prise','$detail','$status')";
	...
	//update row
		...
		$ok=$_GET['edit'];
		$sel="SELECT * FROM men_shirt WHERE id='".$ok."'";
		$exe=$conn->query($sel);
		$row=$exe->fetch_assoc();
		if($name!="" && $set_prise!=""){
			$upd="UPDATE men_shirt SET name='".$image_title."',main_image='".$name."',first_sub_image='".$name1."',second_sub_image='".$name2."',third_sub_image='".$name3."',fourth_sub_image='".$name4."',fifth_sub_image='".$name5."',price='".$set_prise."',detail='".$detail."',status='".$status."' WHERE id='".$ok."'";
			$exe=$conn->query($upd);
			if($exe==true){
